...
test.describe("Search bar search", () => {
  ...
  test("Search with no results term", async ({ page }) => {
    ...
    page.locator(_data_testid_chw_pagebanner_h1).contains(searchTerm);
    await expect(page.locator(_chw_tab_button_title)).toBeAttached();
    await expect(page.locator(_chw_searchlist_count)).contain("(0)");
    await expect(page.locator(_chw_article_item)).toHaveLength(
      expectedPageSize,
    );
  });
  ...
  test("Pagination available", async ({ page }) => {
    ...
    const paginationText = () => page.locator(_pagination).find("span");
    const nextButton = () => page.locator(_aria_label_Next_page_);
    const prevButton = () => page.locator(_aria_label_Previous_page_);

    await expect(paginationText()).contain.text("Page 1");

    // Now paginate forwards.
    nextButton().click();
    // Now check we're on page 2
    await expect(paginationText()).contain.text("Page 2");

    nextButton().click();
    // Now check we're on page 3
    await expect(paginationText()).contain.text("Page 3");
